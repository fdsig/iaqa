...
class html_scraper():
    ...
    def __init__(self):
        ...
        self.scraperapi = '731ccd3ff811ca7644c4ad28d7a8ae09'
        self.wd = '/ava_scraper'
        self.scrape_dir = 'meta_20_08_21'
        
        self.data_dir = os.getcwd()+self.scrape_dir
        print(f'data directory is : {self.data_dir}')
        self.failed = 'failed.txt'
        if self.wd not in os.getcwd():
            self.wd = os.getcwd()+wd
            os.mkdir(self.wd)
            os.chdir(self.wd)
        if self.data_dir not in list(os.scandir()):
            if not os.path.exists(self.data_dir):
                os.mkdir(self.data_dir)
            if not os.path.exists(self.scrape_dir):
                os.mkdir(self.scrape_dir)
            with open(self.data_dir+self.failed, 'w') as fid:
                fid.write('FAILED PAGE SCRAPES')

        self.fid = self.scrape_dir +'/'+'full_dp_challenge_dot_com_data_dict_200821.json'
        print(f'json save in location {self.fid}')
        self.data_dict = { }
        ...
        with open(self.scrape_dir+'/failed.txt','w') as fid:
            fid.writelines('failed requests below')
    ...
    def get_data_dict(self):
        ...
        for chall in tqdm(self.new_challenges,colour=('#FF69B4'),
                          position=0,
                          leave=False):
            self.new_challenges = rng.permutation(self.new_challenges)
            self.chall_id = chall
            ...
            if chall in self.new_challenges:    
                ...
                if 'Request failed' in self.text:
...
def scrape():
    scrape = html_scraper()
    while len(scrape.new_challenges)!=0:
        try: 
            scrape.get_data_dict()
        except:
            print(f'scrape stopped on : {scrape.challenge}')
