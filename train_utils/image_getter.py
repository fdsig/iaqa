...
class Get_Ava:
    ...
    def replace_ava(self):
        ...
        self.mpada_git = [os.path.join(root, name) for root, dirs, files in os.walk(
            '/content/MPADA/', topdown=False) for name in files]
        self.origional_py_files = [
            path for path in self.mpada_git if path[-2:] == 'py']
        self.origional_py_names = [path.split(
            '/')[-1] for path in self.mpada_git if path[-2:] == 'py']
        ...
        if check == 'y':
            ...
            new_py = [py_file.path for py_file in os.scandir(
                '/content/mpada_temp/')]
            import shutil
            print(new_py)
    ...
    def cleanup(self):
        ...
        to_clean = [root_dir.path for root_dir in os.scandir(
            '/content/') if root_dir.name not in acceptable_dir]
        clean_command = 'rm -rf'
    ...
    def google_getter(self):
        ...
        for link in self.args['urls']:
            print(f'{link}\n')
        file_keys += [url.split('/')[-2]
                      for url in self.args['urls'] if 'usp' in url]
    ...
    def ava_txt(self):
        ...
        if len(self.files_crypt) == 44:
            print(
                f'File IDs have sucessfully been obtaniend and now in content/batch_meta/')
    ...
    def download_ava_files(self, **kwargs):
        ...
        if kwargs['own_drive']:
            ...
            self.paths = [i.path for
                          i in
                          os.scandir(
                              '/content/drive/MyDrive/0.AVA/AVA_dataset/batches_zip/')
            ...
            paths = [j.path for i in os.scandir(
                'Images') for j in os.scandir(i)]
...
def import_ava(own_drive=None,
    ...
    pull.google_getter()
    pull.ava_txt()
    pull.download_ava_files(own_drive=own_drive,
                            download=download,
                            full=full,
